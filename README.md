...
аналогичные фильтрам в популярных графических редакторах. Сборка осуществялется при помощи CMake.

## Поддерживаемый формат изображений

Входные и выходные графические файлы в формате [BMP](http://en.wikipedia.org/wiki/BMP_file_format).

В данном приложении используется 24-битный BMP без сжатия и без таблицы цветов. Тип используемого `DIB header` - `BITMAPINFOHEADER`.

Пример файла в нужном формате в статье на Википедии [в разделе "Example 1"](https://en.wikipedia.org/wiki/BMP_file_format#Example_1)
...
В формулах далее каждая компонента цвета
...
соответствует центральный элемент матрицы. (https://en.wikipedia.org/wiki/Kernel_(image_processing)
...
Делит изображение на "number" частей и меняет их местами в случайном порядке.

Если изображение не делится ровно, то сначала оно обрезается.

## Реализация

Задействованные компоненты:
- Структуры для хранения информации об изображении
- Класс, обеспечивающий работу с изображением
- Класс обработки ошибок и исключений
- Фильтры изображений
- Контроллер, управляющий последовательным применением фильтров
- Классы исключений

Общие части выделены через наследование. Все ошибки из-за некорректных входных данных обрабатываются, и выдаётся соответствующее исключение.
